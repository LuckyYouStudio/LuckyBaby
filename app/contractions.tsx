...
export default function Contractions() {
  ...
  const save = () => {
    ...
    const text = `最近一小时 ${recent.length} 次，平均持续 ${fmt(avgDur)}，平均间隔 ${fmt(avgGap)}`;
    dispatch({
      type: 'addLog',
  ...
  return (
    <Screen style={{ padding: space.lg }}>
      <Body2 style={{ textAlign: 'center', marginTop: space.sm }}>感觉肚子发紧发硬时按"开始"，松下来按"结束"。{'\n'}规律不规律，App 帮你算，你只管按。</Body2>
      ...
      <Card style={{ marginTop: space.lg, backgroundColor: regular ? colors.apricotSoft : colors.card, borderColor: regular ? colors.apricot : colors.line }}>
        <Row style={{ justifyContent: 'space-around' }}>
          <View style={{ alignItems: 'center' }}><Text style={{ fontSize: 26, fontWeight: '700', color: colors.ink, fontVariant: ['tabular-nums'] }}>{recent.length}</Text><Caption>最近 1 小时</Caption></View>
          <View style={{ alignItems: 'center' }}><Text style={{ fontSize: 26, fontWeight: '700', color: colors.ink, fontVariant: ['tabular-nums'] }}>{avgDur ? fmt(avgDur) : '—'}</Text><Caption>平均持续</Caption></View>
          <View style={{ alignItems: 'center' }}><Text style={{ fontSize: 26, fontWeight: '700', color: colors.ink, fontVariant: ['tabular-nums'] }}>{avgGap ? fmt(avgGap) : '—'}</Text><Caption>平均间隔</Caption></View>
        </Row>
        {regular ? (
          <Body style={{ marginTop: space.md, color: colors.apricot, fontWeight: '700' }}>已经比较规律了（约每 5 分钟一次、每次约 1 分钟、持续 1 小时）。按医生交代的，可以准备去医院了。</Body>
        ) : (
          <Caption style={{ marginTop: space.md }}>一般到"每 5 分钟一次、每次 1 分钟、持续 1 小时"就该去医院；破水或出血不用等，直接去。</Caption>
        )}
      </Card>
      ...
      <View style={{ alignItems: 'center', marginVertical: space.xl }}>
        <Pressable onPress={toggle} style={({ pressed }) => ({ width: 220, height: 220, borderRadius: 110, backgroundColor: active ? colors.apricot : pressed ? colors.pine : colors.pineSoft, borderWidth: 6, borderColor: active ? colors.apricot : colors.pine, alignItems: 'center', justifyContent: 'center' })}>
          <Text style={{ fontSize: 40, fontWeight: '700', color: active ? colors.onPine : colors.pine, fontVariant: ['tabular-nums'] }}>{active ? fmt(now - active.start) : '开始'}</Text>
          <Text style={{ fontSize: 16, color: active ? colors.onPine : colors.pine, fontWeight: '600', marginTop: 4 }}>{active ? '松了就按结束' : '肚子发紧时按'}</Text>
        </Pressable>
      </View>
        ...
        {done.slice().reverse().map((c, i, arr) => {
          ...
          return (
            <Row key={c.start} style={{ justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: colors.line }}>
              ...
              <Body2>持续 {fmt(c.end - c.start)}</Body2>
              <Body2>{prev ? `间隔 ${fmt(c.start - prev.start)}` : ''}</Body2>
            </Row>
          );
        })}
      </ScrollView>
      ...
      <View style={{ gap: 10, marginBottom: space.xl }}>
        <Button title={done.length ? `保存并返回（${done.length} 次）` : '返回'} onPress={save} />
      </View>
    </Screen>
  );
}
