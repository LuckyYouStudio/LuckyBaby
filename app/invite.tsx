...
export default function Invite() {
  ...
  const shareQr = async () => {
    try {
      ...
      if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(uri, { mimeType: 'image/png', dialogTitle: tr('分享邀请二维码') });
      else await Share.share({ url: uri, message: inviteMessage(code, from) });
    } catch (e: any) { alert(tr('分享失败'), String(e?.message ?? e)); }
  };

  return (
    <Screen style={{ padding: space.lg }}>
      <Body2 style={{ textAlign: 'center', marginTop: space.sm }}>{tr('家人用手机相机扫这个码，或者收到链接后点开，就能加入。')}{'\n'}{tr('加入后能看到什么，由准妈妈决定。')}</Body2>
      ...
      <ViewShot ref={shot} options={{ format: 'png', quality: 1, result: 'tmpfile' }} style={{ alignItems: 'center', marginTop: space.xl }}>
        <Card style={{ alignItems: 'center', padding: space.xl, backgroundColor: '#FFFFFF', borderColor: '#D5D9D1' }}>
          ...
          <Text style={{ fontSize: 13, color: '#4B5750', marginTop: 2 }}>{from ? `${from} 的家庭邀请码` : tr('家庭邀请码')} · 幸运宝贝</Text>
        </Card>
      </ViewShot>

      <View style={{ gap: 10, marginTop: space.xl }}>
        <Button title={copied ? tr('已复制') : tr('复制邀请码')} kind="ghost" onPress={copy} />
        <Button title={tr("分享邀请链接（微信 / 短信）")} onPress={shareText} />
        <Button title={tr("分享二维码图片")} kind="ghost" onPress={shareQr} />
      </View>
      <Caption style={{ marginTop: space.lg, textAlign: 'center' }}>{tr('链接打开是一个网页：已装 App 的人一点就进家庭；没装的人看到邀请码和安装说明。')}</Caption>
