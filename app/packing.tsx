...
export default function Packing() {
  ...
  const groups = [tr('证件'), tr('妈妈'), tr('宝宝')];
    ...
    <Screen>
      <ScrollView contentContainerStyle={{ padding: space.lg, paddingBottom: 60 }} keyboardShouldPersistTaps="handled">
        <Row style={{ justifyContent: 'space-between' }}>
          <Body2>{tr('建议孕 34 周前备齐。勾一下代表准备好了，会显示是谁准备的。')}</Body2>
        </Row>
        ...
        <Caption style={{ marginTop: 4 }}>{done} / {items.length} {tr('已备好')}</Caption>

        {groups.map((g) => (
          <Section key={g} title={tr(g)} right={!readonly ? <Pressable onPress={() => { setAdding(g); setText(''); }}><Caption style={{ color: colors.pine }}>{tr('添加')}</Caption></Pressable> : undefined}>
              ...
              {items.filter((i) => i.group === g).map((it, idx) => {
                ...
                return (
                  <Pressable key={it.id} disabled={readonly} onPress={() => dispatch({ type: 'togglePacking', id: it.id, byId: me.id })} style={{ flexDirection: 'row', alignItems: 'center', padding: space.md, paddingHorizontal: space.lg, borderTopWidth: idx ? 1 : 0, borderTopColor: colors.line }}>
                    ...
                    <Body style={{ flex: 1, color: it.done ? colors.ink3 : colors.ink, textDecorationLine: it.done ? 'line-through' : 'none' }}>{tr(it.text)}</Body>
            ...
            {adding === g && (
              <Card style={{ marginTop: space.sm }}>
                <Field label={tr("加一项")} value={text} onChange={setText} placeholder={tr("例如：束腹带")} />
                <Row style={{ gap: 8 }}>
                  <Button title={tr("添加")} small onPress={() => { if (text.trim()) dispatch({ type: 'addPacking', group: g, text: text.trim() }); setAdding(null); }} />
                  <Button title={tr("取消")} small kind="ghost" onPress={() => setAdding(null)} />
                </Row>
              </Card>
            )}
          </Section>
        ))}
        <Caption style={{ marginTop: space.xl, textAlign: 'center' }}>{tr('清单按常见医院要求整理，以你产检医院的通知为准。')}</Caption>
      </ScrollView>
    </Screen>
  );
}
