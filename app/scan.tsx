  ...
  if (!perm.granted) {
    return (
      <Screen style={{ padding: space.xl, justifyContent: 'center' }}>
        <Body2 style={{ textAlign: 'center', marginBottom: space.lg }}>{tr('需要相机权限来扫二维码。也可以回去手动输入 6 位邀请码。')}</Body2>
        <Button title={tr("允许使用相机")} onPress={request} />
        <Button title={tr("返回")} kind="ghost" onPress={() => router.back()} style={{ marginTop: 10 }} />
      </Screen>
    );
  }
  return (
    <Screen>
      ...
      <View style={{ position: 'absolute', left: 0, right: 0, top: '20%', alignItems: 'center' }}>
        <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600', backgroundColor: 'rgba(0,0,0,0.45)', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 }}>{tr('对准家人分享的二维码')}</Text>
      </View>
      ...
      <View style={{ position: 'absolute', left: space.lg, right: space.lg, bottom: 40 }}>
        <Button title={tr("取消")} kind="ghost" onPress={() => router.back()} style={{ backgroundColor: 'rgba(255,255,255,0.9)' }} />
      </View>
    </Screen>
  );
}
